...
class ArtnetDataPacket implements ArtnetPacket {
  ...
  ByteData packet = ByteData(512);
  Uint8List data = Uint8List(512);
  ...
  set ArtnetProtVerHi(int value) =>
      this.packet.setUint8(ArtnetProtVerHiIndex, value);

  int get ArtnetProtVerLo => this.packet.getUint8(ArtnetProtVerLoIndex);
  set ArtnetProtVerLo(int value) =>
      this.packet.setUint8(ArtnetProtVerLoIndex, value);

  int get ArtnetProtVersion => this.packet.getUint16(ArtnetProtVerHiIndex);
  set ArtnetProtVersion(int value) =>
      this.packet.setUint16(ArtnetProtVerHiIndex, value);
  ...
  List<int> get dmx =>
      this.packet.buffer.asUint8List(dataIndex, this.dmxLength);

  List<int> get udpPacket =>
      this.packet.buffer.asUint8List(0, size + this.dmxLength);

  @override
  String toString() {
    String string = "***$type***\n";
    string += "Id: " +
        String.fromCharCodes(this.packet.buffer.asUint8List(0, 8)) +
        "\n";
    string += "Opcode: 0x" +
        this.packet.getUint16(ArtnetOpCodeIndex).toRadixString(16) +
        "\n";
    ...
    for (var i = 1; i <= this.dmxLength; i++) {
      ...
      String tempString =
          " " + (i).toString() + ":" + this.dmx[i - 1].toString() + " ";
...
class ArtnetPollPacket implements ArtnetPacket {
  ...
  bool get talkToMeVlcTransmissionEnable =>
      ((this.talkToMe & talkToMeVlcTransmissionEnableMask) == 0);
  set talkToMeVlcTransmissionEnable(bool value) => (value)
      ? this.talkToMe &= ~talkToMeVlcTransmissionEnableMask
      : this.talkToMe |= talkToMeVlcTransmissionEnableMask;

  bool get talkToMeDiagnosticsEnable =>
      ((this.talkToMe & talkToMeDiagnosticsEnableMask) != 0);
  set talkToMeDiagnosticsEnable(bool value) => (value)
      ? this.talkToMe |= talkToMeDiagnosticsEnableMask
      : this.talkToMe &= ~talkToMeDiagnosticsEnableMask;

  int get talkToMeDiagnosticsTransmission =>
      (this.talkToMe & talkToMeDiagnosticsTransmissionMask) >> 3;
  set talkToMeDiagnosticsTransmission(int value) =>
      (value == talkToMeDiagnosticsTransmissionOptionUnicast)
          ? this.talkToMe |= talkToMeDiagnosticsTransmissionMask
          : this.talkToMe &= ~talkToMeDiagnosticsTransmissionMask;

  int get talkToMePollReplyOption =>
      (this.talkToMe & talkToMePollReplyOptionMask) >> 1;
  set talkToMePollReplyOption(int value) =>
      (value == talkToMePollReplyOptionOnChange)
  ...
  String toString() {
    ...
    string += "*** VLC Transmission: " +
        ((this.talkToMeVlcTransmissionEnable) ? "Enabled" : "Disabled") +
        "\n";
    string += "*** Diagnostics: " +
        ((this.talkToMeDiagnosticsEnable) ? "Enabled" : "Disabled") +
        "\n";
    string += "*** Diagnostics are " +
        ((this.talkToMeDiagnosticsTransmission ==
                talkToMeDiagnosticsTransmissionOptionBroadcast)
            ? "broadcast"
            : "unicast") +
        "\n";
...
class ArtnetPollReplyPacket implements ArtnetPacket {
  ...
  static const blizzardCodeSize =
      blizzardLoveSize + blizzardTypeSize + blizzardPrideSize;
  ...
  static const blizzardLoveIndex =
      nodeReportIndex + blizzardReportSize; //Blizzard Pro Proprietary Info
  static const blizzardTypeIndex =
      blizzardLoveIndex + blizzardLoveSize; //Blizzard Pro Proprietary Info
  static const blizzardPrideIndex =
      blizzardTypeIndex + blizzardTypeSize; //Blizzard Pro Proprietary Info
  ...
  List<int> get ip =>
      this.packet.buffer.asUint8List(ipAddressIndex, ipAddressSize);
  ...
  set netSwitch(int value) =>
      this.packet.setUint8(netSwitchIndex, value & netSwitchMask);

  int get subSwitch => this.packet.getUint8(subSwitchIndex);
  set subSwitch(int value) =>
      this.packet.setUint8(subSwitchIndex, value & subSwitchMask);

  int get universe =>
      this.netSwitch << 16 | this.subSwitch << 8 | this.swOut[0];
  set universe(int value) {
  ...
  int get status1IndicatorState =>
      (this.status1 & status1IndicatorStateMask) >> 6;
  ...
  int get status1ProgrammingAuthority =>
      (this.status1 & status1ProgrammingAuthorityMask) >> 4;
  ...
  int get status1FirmwareBoot =>
      (this.status1 & status1ProgrammingAuthorityMask) >> 2;
  ...
  set status1RdmCapable(bool value) => (value)
      ? this.status1 |= status1RdmCapableMask
      : this.status1 &= ~status1RdmCapableMask;

  bool get status1UbeaPresent => ((this.status1 & status1UbeaPresentMask) != 0);
  set status1UbeaPresent(bool value) => (value)
      ? this.status1 |= status1UbeaPresentMask
      : this.status1 &= ~status1UbeaPresentMask;
  ...
  String get shortName => String.fromCharCodes(
      this.packet.buffer.asUint8List(shortNameIndex, shortNameSize));
  set shortName(String value) {
    ...
    this.packet.setUint8(
        shortNameIndex + shortNameSize - 1, 0); //Null terminate just in case
  }

  String get longName => String.fromCharCodes(
      this.packet.buffer.asUint8List(longNameIndex, longNameSize));
  set longName(String value) {
    ...
    this.packet.setUint8(
        longNameIndex + longNameSize - 1, 0); //Null terminate just in case
  }

  String get nodeReport => String.fromCharCodes(
      packet.buffer.asUint8List(nodeReportIndex, nodeReportSize));
  set nodeReport(String value) {
    ...
    this.packet.setUint8(
        nodeReportIndex + nodeReportSize - 1, 0); //Null terminate just in case
  }
  ...
  int get blizzardType => (this.isBlizzardDevice)
      ? this.packet.getUint16(blizzardTypeIndex, Endian.little)
      : 0x0000;
  ...
  List<int> get portTypes =>
      this.packet.buffer.asUint8List(portTypesIndex, portTypesSize);
  ...
  bool getPortTypesOutputArtnetAble(int index) => (index >= portTypesSize)
      ? false
      : ((this.portTypes[index] & portTypesOutputArtnetAbleMask) != 0x00);
  ...
  bool getPortTypesInputArtnetAble(int index) => (index >= portTypesSize)
      ? false
      : ((this.portTypes[index] & portTypesInputArtnetAbleMask) != 0x00);
  ...
  int getPortTypesProtocol(int index) => (index >= portTypesSize)
      ? 0x00
  ...
  List<int> get goodInput =>
      this.packet.buffer.asUint8List(goodInputIndex, goodInputSize);
  ...
  bool getGoodInputDataReceived(int index) => (index >= goodInputSize)
      ? false
      : ((this.goodInput[index] & goodInputDataReceivedMask) != 0x00);
  void setGoodInputDataReceived(int index, bool value) {
    if (index >= goodInputSize) return;

    (value)
        ? this.goodInput[index] |= goodInputDataReceivedMask
        : this.goodInput[index] &= ~goodInputDataReceivedMask;
  }

  bool getGoodInputIncludesTestPackets1(int index) => (index >= goodInputSize)
      ? false
      : ((this.goodInput[index] & goodInputIncludesTestPackets1Mask) != 0x00);
  ...
  bool getGoodInputIncludesSIPs(int index) => (index >= goodInputSize)
      ? false
      : ((this.goodInput[index] & goodInputIncludesSIPsMask) != 0x00);
  void setGoodInputIncludesSIPs(int index, bool value) {
    if (index >= goodInputSize) return;

    (value)
        ? this.goodInput[index] |= goodInputIncludesSIPsMask
        : this.goodInput[index] &= ~goodInputIncludesSIPsMask;
  }

  bool getGoodInputIncludesTestPackets2(int index) => (index >= goodInputSize)
      ? false
      : ((this.goodInput[index] & goodInputIncludesTestPackets2Mask) != 0x00);
  ...
  bool getGoodInputInputDisable(int index) => (index >= goodInputSize)
      ? false
      : ((this.goodInput[index] & goodInputInputDisableMask) != 0x00);
  void setGoodInputInputDisable(int index, bool value) {
    if (index >= goodInputSize) return;

    (value)
        ? this.goodInput[index] |= goodInputInputDisableMask
        : this.goodInput[index] &= ~goodInputInputDisableMask;
  }

  bool getGoodInputReceiveErrorDetected(int index) => (index >= goodInputSize)
      ? false
      : ((this.goodInput[index] & goodInputReceiveErrorDetectedMask) != 0x00);
  ...
  List<int> get goodOutput =>
      this.packet.buffer.asUint8List(goodOutputIndex, goodOutputSize);
  ...
  bool getGoodOutputDataTransmiting(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputDataTransmitingMask) != 0x00);
  ...
  bool getGoodOutputIncludesTestPackets1(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputIncludesTestPackets1Mask) != 0x00);
  ...
  bool getGoodOutputIncludesSIPs(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputIncludesSIPsMask) != 0x00);
  ...
  bool getGoodOutputIncludesTestPackets2(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputIncludesTestPackets2Mask) != 0x00);
  ...
  bool getGoodOutputIsMerging(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputIsMergingMask) != 0x00);
  void setGoodOutputIsMerging(int index, bool value) {
    if (index >= goodOutputSize) return;

    (value)
        ? this.goodOutput[index] |= goodOutputIsMergingMask
        : this.goodOutput[index] &= ~goodOutputIsMergingMask;
  }

  bool getGoodOutputShortDetected(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputShortDetectedMask) != 0x00);
  ...
  bool getGoodOutputMergeIsLTP(int index) => (index >= goodOutputSize)
      ? false
      : ((this.goodOutput[index] & goodOutputMergeIsLTPMask) != 0x00);
  void setGoodOutputMergeIsLTP(int index, bool value) {
    if (index >= goodOutputSize) return;

    (value)
        ? this.goodOutput[index] |= goodOutputMergeIsLTPMask
        : this.goodOutput[index] &= ~goodOutputMergeIsLTPMask;
  }

  /* TODO make this an int */
  int getGoodOutputProtocol(int index) => (index >= goodOutputSize)
      ? 0x00
  ...
  List<int> get bindIp =>
      this.packet.buffer.asUint8List(bindIpIndex, bindIpSize);
  ...
  bool get status2IsSquawking =>
      ((this.status2 & status2IsSquawkingMask) != 0x00);
  set status2IsSquawking(bool value) => (value)
      ? this.status2 |= status2IsSquawkingMask
      : this.status2 &= ~status2IsSquawkingMask;

  bool get status2ProtocolSwitchable =>
      ((this.status2 & status2ProtocolSwitchableMask) != 0x00);
  set status2ProtocolSwitchable(bool value) => (value)
      ? this.status2 |= status2ProtocolSwitchableMask
      : this.status2 &= ~status2ProtocolSwitchableMask;

  bool get status215BitSupport =>
      ((this.status2 & status215BitSupportMask) != 0x00);
  set status215BitSupport(bool value) => (value)
      ? this.status2 |= status215BitSupportMask
      : this.status2 &= ~status215BitSupportMask;

  bool get status2DHCPCapable =>
      ((this.status2 & status2DHCPCapableMask) != 0x00);
  set status2DHCPCapable(bool value) => (value)
      ? this.status2 |= status2DHCPCapableMask
      : this.status2 &= ~status2DHCPCapableMask;

  bool get status2IpIsSetManually =>
      ((this.status2 & status2IpIsSetManuallyMask) == 0x00);
  set status2IpIsSetManually(bool value) => (value)
      ? this.status2 &= ~status2IpIsSetManuallyMask
      : this.status2 |= status2IpIsSetManuallyMask;

  bool get status2HasWebConfigurationSupport =>
      ((this.status2 & status2HasWebConfigurationSupportMask) != 0x00);
  ...
  String toString() {
    ...
    string += "Version: " +
        this.versionInfoH.toString() +
        "." +
        this.versionInfoL.toString() +
        "\n";
    ...
    string += "*** Firmware Boot: " +
        ((this.status1FirmwareBoot == status1FirmwareBootOptionRom)
            ? "ROM boot"
            : "Normal boot (from flash)") +
        "\n";
    string += "*** RDM Capable: " +
        ((this.status1RdmCapable) ? "Capable" : "Not Capable") +
        "\n";
    string += "*** UBEA Present: " +
        ((this.status1UbeaPresent) ? "Present" : "Not Present or Currupt") +
        "\n";
    string +=
        "ESTA Manufacturer Code: 0x" + this.estaMan.toRadixString(16) + "\n";
    ...
    for (var i = 0; i < portTypesSize; i++) {
      string += "*** " + i.toString() + ":\n";
      string += "*** *** Artnet Output: " +
          ((this.getPortTypesInputArtnetAble(i)) ? "Enabled" : "Disabled") +
          "\n";
      string += "*** *** Artnet Input: " +
          ((this.getPortTypesOutputArtnetAble(i)) ? "Enabled" : "Disabled") +
          "\n";
      string += "*** *** Protocol: ";
    ...
    for (var i = 0; i < goodOutputSize; i++) {
      ...
      string += "*** *** Data Transmitting: " +
          ((this.getGoodOutputDataTransmiting(i)) ? "True" : "False") +
          "\n";
      string += "*** *** Channel Includes DMX Test Packets 1: " +
          ((this.getGoodOutputIncludesTestPackets1(i)) ? "True" : "False") +
          "\n";
      string += "*** *** Channel Includes DMX SIP's: " +
          ((this.getGoodOutputIncludesSIPs(i)) ? "True" : "False") +
          "\n";
      string += "*** *** Channel Includes DMX Test Packets 2: " +
          ((this.getGoodOutputIncludesTestPackets2(i)) ? "True" : "False") +
          "\n";
      string += "*** *** Merge: " +
          ((this.getGoodOutputIsMerging(i)) ? "Disabled" : "Enabled") +
          "\n";
      string += "*** *** DMX Short: " +
          ((this.getGoodOutputShortDetected(i)) ? "Detected" : "Not Detected") +
          "\n";
      string += "*** *** Merge is LTP: " +
          ((this.getGoodOutputMergeIsLTP(i)) ? "True" : "False") +
          "\n";
      string += "*** *** Protocol: " +
          ((this.getGoodOutputProtocol(i) == goodOutputProtocolOptionSacn)
              ? "sACN"
              : "Art-Net") +
          "\n";
    }
    ...
    string += "*** Node is Squawking: " +
        ((this.status2IsSquawking) ? "True" : "False") +
        "\n";
    ...
    string += "*** Node Supports 15 Bit Port-Address (Art-Net 3 or 4): " +
        ...
        "\n";
    string += "*** Node Supports DHCP: " +
        ((this.status2DHCPCapable) ? "True" : "False") +
        "\n";
    string += "*** Node's ip is set " +
        ((this.status2IpIsSetManually) ? "manually" : "by DHCP") +
        "\n";
    string += "*** Node Supports Web Configurations: " +
        ((this.status2HasWebConfigurationSupport) ? "True" : "False") +
        "\n";
    return string;
  }
...
class ArtnetAddressPacket implements ArtnetPacket {
  ...
  bool get programNetSwitchEnable =>
      (this.netSwitch & programSwitchMask) != 0x00;
  set programNetSwitchEnable(bool value) => (value)
      ? this.netSwitch |= programSwitchMask
      : this.netSwitch &= ~programSwitchMask;

  int get subSwitch => this.packet.getUint8(subSwitchIndex);
  set subSwitch(int value) => this.packet.setUint8(subSwitchIndex, value);

  bool get programSubSwitchEnable =>
      (this.subSwitch & programSwitchMask) != 0x00;
  set programSubSwitchEnable(bool value) => (value)
      ? this.subSwitch |= programSwitchMask
      : this.subSwitch &= ~programSwitchMask;
  ...
  void setProgramSwInEnable(int index, bool value) {
    ...
    (value)
        ? this.swIn[index] |= programSwitchMask
        : this.swIn[index] &= ~programSwitchMask;
  }
  ...
  void setProgramSwOutEnable(int index, bool value) {
    ...
    (value)
        ? this.swOut[index] |= programSwitchMask
        : this.swOut[index] &= ~programSwitchMask;
  }
  ...
  bool get programUniverseEnable => (this.programNetSwitchEnable ||
      this.programSubSwitchEnable ||
      this.getProgramSwOutEnable(0));
...
class ArtnetIpProgPacket implements ArtnetPacket {
  ...
  bool get commandProgrammingEnable =>
      ((this.command & commandProgrammingEnableMask) != 0x00);
  set commandProgrammingEnable(bool value) => (value)
      ? this.command |= commandProgrammingEnableMask
      : this.command &= ~commandProgrammingEnableMask;

  bool get commandDHCPEnable =>
      ((this.command & commandDHCPEnableMask) != 0x00);
  set commandDHCPEnable(bool value) => (value)
      ? this.command |= commandDHCPEnableMask
      : this.command &= ~commandDHCPEnableMask;

  bool get commandResetIpSubnetPortToDefault =>
      ((this.command & commandResetIpSubnetPortToDefaultMask) != 0x00);
  ...
  set commandProgramIp(bool value) => (value)
      ? this.command |= commandProgramIpMask
      : this.command &= ~commandProgramIpMask;

  bool get commandProgramSubnet =>
      ((this.command & commandProgramSubnetMask) != 0x00);
  set commandProgramSubnet(bool value) => (value)
      ? this.command |= commandProgramSubnetMask
      : this.command &= ~commandProgramSubnetMask;

  bool get commandProgramPort =>
      ((this.command & commandProgramPortMask) != 0x00);
  set commandProgramPort(bool value) => (value)
      ? this.command |= commandProgramPortMask
      : this.command &= ~commandProgramPortMask;
  ...
  List<int> get progIp =>
      this.packet.buffer.asUint8List(progIpHiIndex, progIpSize);
  set ip(List<int> value) {
  ...
  List<int> get progSm =>
      this.packet.buffer.asUint8List(progSmHiIndex, progSubnetSize);
  ...
  String toString() {
    ...
    string += "Command: \n";
    string += "*** Programming: " +
        ((this.commandProgrammingEnable) ? "Enabled" : "Disabled") +
        "\n";
    string += "*** DHCP: " +
        ((this.commandDHCPEnable) ? "Enabled" : "Disabled") +
        "\n";
    string += "*** Node is set to " +
        ((this.commandResetIpSubnetPortToDefault) ? "reset " : "NOT reset ") +
        "ip, netmask and port back to default\n";
    string += "*** Node is set to " +
        ((this.commandProgramIp) ? "program " : "NOT program ") +
        " the ip\n";
    string += "*** Node is set to " +
        ((this.commandProgramSubnet) ? "program " : "NOT program ") +
        " the subnet\n";
    string += "*** Node is set to " +
        ((this.commandProgramPort) ? "program " : "NOT program ") +
        " the port\n";
...
class ArtnetIpProgReplyPacket implements ArtnetPacket {
  ...
  set dhcpEnabled(bool value) => (value)
      ? this.status |= statusDHCPEnabledMask
      : this.status &= ~statusDHCPEnabledMask;
  ...
  String toString() {
    ...
    string += "*** DHCP Enable: " +
        ((this.dhcpEnabled) ? "Enabled" : "Disabled") +
        "\n";
    return string;
  }
...
class ArtnetCommandPacket implements ArtnetPacket {
  ...
  List<int> get data =>
      this.packet.buffer.asUint8List(dataIndex, this.dataLength);
...
class ArtnetFirmwareMasterPacket implements ArtnetPacket {
  ...
  set firmwareLength3(int value) =>
      this.packet.setUint8(firmwareLength3Index, value);

  int get firmwareLength2 => this.packet.getUint8(firmwareLength2Index);
  set firmwareLength2(int value) =>
      this.packet.setUint8(firmwareLength2Index, value);

  int get firmwareLength1 => this.packet.getUint8(firmwareLength1Index);
  set firmwareLength1(int value) =>
      this.packet.setUint8(firmwareLength1Index, value);

  int get firmwareLength0 => this.packet.getUint8(firmwareLength0Index);
  set firmwareLength0(int value) =>
      this.packet.setUint8(firmwareLength0Index, value);

  int get firmwareLength => this.packet.getUint64(firmwareLength3Index);
  set firmwareLength(int value) =>
      this.packet.setUint64(firmwareLength3Index, value);
  ...
  String toString() {
    ...
    string += "Firmware Length (words - Int16): " +
        this.firmwareLength.toString() +
        "\n";
    string += "Data:";
    for (var i = 0; i < dataSize; i++) {
      if ((i % 16) == 0) {
...
class ArtnetBeepBeepPacket implements ArtnetPacket {
  ...
  ArtnetBeepBeepPacket({required int uuid, List<int>? packet}) {
    this.packet = new ByteData(size);
    if (packet != null) {
      for (var i = 0; i < size; i++) {
        if (packet.length <= i) return;
